...
export default function PriceBreakdownCard({ breakdown, error }: Props) {
  ...
  if (!breakdown) {
    return (
      <Paper sx={{ p: 3 }}>
        <Typography color="text.secondary">Fill in the itinerary and click Calculate to see the cost breakdown.</Typography>
      </Paper>
    );
  }

  const text = [
    `Distance: ${formatNumber(breakdown.distance.totalKm, 0)} km`,
    `Fuel: ${formatPkr(breakdown.fuel.cost)}`,
    `Vehicle rent: ${formatPkr(breakdown.vehicleRent.cost)}`,
    `Hotels: ${formatPkr(breakdown.hotels.total)}`,
    `Jeeps: ${formatPkr(breakdown.jeeps.total)}`,
    ...
    <Paper sx={{ p: 3 }}>
      <Stack spacing={2}>
        <Stack direction="row" sx={{ justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <Chip size="small" label={breakdown.distance.source === 'google' ? 'Google Maps' : 'Manual matrix'} color="warning" />
        </Stack>

        {breakdown.distance.missingLegs && breakdown.distance.missingLegs.length > 0 && (
          <Alert severity="warning">Missing distance legs: {breakdown.distance.missingLegs.join(', ')}</Alert>
        )}

        <Box>
          <Typography variant="subtitle2" gutterBottom>Distance</Typography>
          <Line label="Total" value={`${formatNumber(breakdown.distance.totalKm, 0)} km`} bold />
        </Box>
        <Divider />
        <Line label={`Fuel (${formatNumber(breakdown.fuel.liters, 1)} L)`} value={formatPkr(breakdown.fuel.cost)} />
        <Line label="Vehicle rent" value={formatPkr(breakdown.vehicleRent.cost)} />
        <Line label="Driver + tolls" value={formatPkr(breakdown.provisions.driver + breakdown.provisions.tolls)} />

        {breakdown.hotels.lineItems.map((item) => (
          <Line
            key={`${item.destination}-${item.hotelName}`}
            label={`${item.hotelName} (${item.rooms}r × ${item.nights}n)`}
            value={formatPkr(item.cost)}
          />
        ))}

        {breakdown.jeeps.lineItems.map((item) => (
          <Line key={item.name} label={`${item.name} × ${item.quantity}`} value={formatPkr(item.cost)} />
        ))}
        ...
        <Line label={`Margin (${breakdown.margin.percent}%)`} value={formatPkr(breakdown.margin.amount)} />
        ...
        <Button startIcon={<ContentCopyIcon />} variant="outlined" onClick={() => navigator.clipboard.writeText(text)}>
          Copy breakdown
        </Button>
      </Stack>
    </Paper>
